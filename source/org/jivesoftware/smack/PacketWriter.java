...
class PacketWriter {
	...
	/**
	 * Timestamp when the last stanza was sent to the server. This information is
	 * used by the keep alive process to only send heartbeats when the connection
	 * has been idle.
	 */
	 ...
	 * sent to the server every 30 seconds (by default) since the last stanza was
	 * sent to the server.
	 */
	void startKeepAliveProcess() {
		...
		// open.
		/*
		 * int keepAliveInterval = SmackConfiguration.getKeepAliveInterval(); if
		 * (keepAliveInterval > 0) { KeepAliveTask task = new
		 * KeepAliveTask(keepAliveInterval); keepAliveThread = new Thread(task);
		 * task.setThread(keepAliveThread); keepAliveThread.setDaemon(true);
		 * keepAliveThread.setName("Smack Keep Alive (" +
		 * connection.connectionCounterValue + ")"); keepAliveThread.start(); }
		 */
	}
	...
	private void writePackets(Thread thisThread) throws Exception {
		try {
			...
			// possible
			// we won't have time to entirely flush it before the socket is forced
			// closed
	 ...
	 * 
	 * private int delay; private Thread thread;
	 * 
	 * public KeepAliveTask(int delay) { this.delay = delay; }
	 * 
	 * protected void setThread(Thread thread) { this.thread = thread; }
	 * 
	 * public void run() { try { // Sleep a minimum of 30 seconds plus delay
	 * before sending first heartbeat. This will give time to // properly finish
	 * TLS negotiation and then start sending heartbeats. Thread.sleep(30000 +
	 * delay); } catch (InterruptedException ie) { // Do nothing } while (!done &&
	 * keepAliveThread == thread) { synchronized (writer) { // Send heartbeat if
	 * no packet has been sent to the server for a given time if
	 * (System.currentTimeMillis() - lastActive >= delay) { try {
	 * writer.write(" "); writer.flush(); } catch (Exception e) { // Do nothing }
	 * } } try { // Sleep until we should write the next keep-alive.
	 * Thread.sleep(delay); } catch (InterruptedException ie) { // Do nothing } }
	 * } }
	 */
}
