...
class ServerTrustManager implements X509TrustManager {
	...
	public void checkServerTrusted(X509Certificate[] x509Certificates, String arg1)
		...
		if (configuration.isNotMatchingDomainCheckEnabled()) {
			...
			// Check if the certificate uses a wildcard indicating that subdomains are
			// valid
	 ...
	 * and it can also be defined in the subjectAltName extension of type "xmpp".
	 * When the extension is being used then the identity defined in the extension
	 * in going to be returned. Otherwise, the value stored in the subjectDN is
	 * returned.
	 * 
	...
	private static List<String> getSubjectAlternativeNames(
		...
		try {
			...
			/*
			 * for (List item : altNames) { Integer type = (Integer) item.get(0); if
			 * (type == 0) { // Type OtherName found so return the associated value
			 * try { // Value is encoded using ASN.1 so decode it to get the server's
			 * identity ASN1InputStream decoder = new ASN1InputStream((byte[])
			 * item.toArray()[1]); DEREncodable encoded = decoder.readObject();
			 * encoded = ((DERSequence) encoded).getObjectAt(1); encoded =
			 * ((DERTaggedObject) encoded).getObject(); encoded = ((DERTaggedObject)
			 * encoded).getObject(); String identity = ((DERUTF8String)
			 * encoded).getString(); // Add the decoded server name to the list of
			 * identities identities.add(identity); } catch
			 * (UnsupportedEncodingException e) { // Ignore } catch (IOException e) {
			 * // Ignore } catch (Exception e) { e.printStackTrace(); } } // Other
			 * types are not good for XMPP so ignore them
