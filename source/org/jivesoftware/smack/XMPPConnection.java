...
public class XMPPConnection extends Connection {
	...
	/**
	 * Flag that indicates if the user was authenticated with the server when the
	 * connection to the server was closed (abruptly or not).
	 */
	 ...
	 * will be performed to determine the IP address and port corresponding to the
	 ...
	 * <p>
	 * <p/>
	 * Note that XMPPConnection constructors do not establish a connection to the
	 * server and you must call {@link #connect()}.
	 * <p>
	 * <p/>
	 * The CallbackHandler will only be used if the connection requires the client
	 * provide an SSL certificate to the server. The CallbackHandler must handle
	 * the PasswordCallback to prompt for a password to unlock the keystore
	 ...
	 * {@link #XMPPConnection(String,CallbackHandler)} does, but with no callback
	 * handler for password prompting of the keystore. This will work in most
	 * cases, provided the client is not required to provide a certificate to the
	 * server.
	 * 
	 * @param serviceName
	 *          the name of the XMPP server to connect to; e.g.
	...
	/**
	 * Creates a new XMPP connection in the same way
	 * {@link #XMPPConnection(ConnectionConfiguration,CallbackHandler)} does, but
	 * with no callback handler for password prompting of the keystore. This will
	 * work in most cases, provided the client is not required to provide a
	...
	public synchronized void login(String username, String password,
		...
		// Create the roster if it is not a reconnection or roster already created
		// by getRoster()
		/*
		 * if (this.roster == null) { if(rosterStorage==null){ this.roster = new
		 * Roster(this); } else{ this.roster = new Roster(this,rosterStorage); } }
		 * if (config.isRosterLoadedAtLogin()) { this.roster.reload(); }
		 */
	 ...
	 * stream to the XMPP server. The shutdown logic will be used during a planned
	 * disconnection or when dealing with an unexpected disconnection. Unlike
	 * {@link #disconnect()} the connection's packet reader, packet writer, and
	...
	public void sendPacket(Packet packet) {
		...
		if (packetWriter == null) {
			throw new NullPointerException("Packet writer is null.");
		}
	 ...
	 * thread that writes packets will be used to invoke the listeners. Therefore,
	 * each packet listener should complete all operations quickly or use a
...
//			}
	...
	private void initConnection() throws Exception {
		...
		try {
			if (isFirstInitialization) {
				...
				// If debugging is enabled, we should start the thread that will listen
				// for
	...
	*/
		...
		try {
			...
			throw new XMPPException("XMPPError establishing connection with server.",
					new XMPPError(XMPPError.Condition.remote_server_error,
							"XMPPError establishing connection with server."), ioe);
		}
		...
		if (context == null) {
			...
			context.init(kms,
					new javax.net.ssl.TrustManager[]{new ServerTrustManager(
							getServiceName(), config)}, new java.security.SecureRandom());
		}
		Socket plain = socket;
	 ...
	 * can be reduced up to 90%. Therefore, stream compression is ideal when using
	 * a slow speed network connection. However, the server and the client will
	 * need to use more CPU time in order to un/compress network data so under
	 * high load the server performance might be affected.
	 * <p>
	 * <p/>
	 ...
	 * using TLS then negotiation of stream compression can only happen after TLS
	 * was negotiated. If TLS compression is being used the stream compression
	 * should not be used.
	 */
	...
	/**
	 * Notifies the XMPP connection that stream compression was denied so that the
	 ...
	 * only if the previous connection state was logged (authenticated). It
	 * basically creates and maintains a socket connection to the server.
	 * <p>
	 * <p/>
	 ...
	 * 
	 * @throws XMPPException
	 *           if an error occurs while trying to establish the connection. Two
	 *           possible errors can occur which will be wrapped by an
	 *           XMPPException -- UnknownHostException (XMPP error code 504), and
	 *           IOException (XMPP error code 502). The error codes and wrapped
